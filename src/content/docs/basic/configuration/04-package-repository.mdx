...
2.  Click the **Edit** link (displayed with a pencil icon) on the right side of the row.
...
To toggle a repository's active status:

1.  Locate the repository in the list.
2.  Click on the circle icon in the **ENABLED** column to toggle between enabled (filled circle) and disabled (empty circle).
3.  The change takes effect immediately.

:::note
Disabling a repository will prevent machines from using it as a package source. Ensure that critical repositories remain enabled to avoid issues with package installation and updates.
:::
