...
<Aside>
	At least 1+N machines are required, where N is the number of K8S nodes (minimum 1). The first machine is dedicated to the OtterScale Control Node. The remaining N machines are BMC-managed servers for the K8S cluster.
</Aside>
...
It is required to open the firewall on each node.
